...
# -*- coding: utf-8 -*-
"""
Dual-Modality Cross-Attention Fusion Model for Spermatogenic Activity Prediction
Model Architecture:
...
6. Molecular virtual screening pipeline for active compound discovery
"""
import torch
...
# Relative File Paths (No Absolute Paths for GitHub)
TRAIN_DRUG_FEAT_PATH = "spr_grover_vectors.npz"
TRAIN_DNA_FEAT_PATH = "spr_dna_embeddings.npz"
TRAIN_LABEL_PATH = "spr_data_g4ldb_valid.csv"

SCREEN_DRUG_FEAT_PATH = "fert_selected_features.npz"
SCREEN_DNA_FEAT_PATH = "dna_embeddings.npz"
SCREEN_SMILES_PATH = "fert_selected_smiles.csv"

# Model & Result Save Paths
MODEL_SAVE_PATH = "spr_trained_model.pth"
SCREEN_RESULT_CSV_PATH = "spr_selected_smiles.csv"
SCREEN_RESULT_FEAT_PATH = "spr_selected_features.npz"
...
class FertilityDataset(Dataset):
    """
    Dual-modality dataset for drug fingerprint + DNA embedding + binary spermatogenic label
    """
...
class MultiHeadSelfAttention(nn.Module):
    """Single modality multi-head self-attention module for feature enhancement"""
...
class CrossAttention(nn.Module):
    """Cross-attention module for inter-modal feature interaction"""
...
class SEBlock(nn.Module):
    """Squeeze-and-Excitation Channel Attention Module for adaptive feature weighting"""
...
class AttFertilityModel(nn.Module):
    """Dual-modality cross-attention fusion model for spermatogenic prediction"""
    def __init__(self, drug_dim=3200, dna_dim=768, hidden=HIDDEN_DIM, heads=ATTN_HEADS, dropout=DROPOUT_RATE):
        super().__init__()
        # Single modality feature projection
    ...
    def forward(self, drug, dna):
        # Feature projection
        drug_f = self.drug_proj(drug).unsqueeze(1)  # [B, 1, H]
        dna_f = self.dna_proj(dna).unsqueeze(1)    # [B, 1, H]
        ...
        # Final feature fusion and prediction
...
class WeightedFocalLoss(nn.Module):
    ...
    def __init__(self, alpha=0.75, gamma=2.0, pos_weight=3.0):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, inputs, targets):
        ...
        # Weight mask for positive/negative sample balance
...
def evaluate_detailed(model, loader, device, dataset_name="Test"):
    """Detailed classification report with per-class precision, recall and F1"""
    model.eval()
...
def evaluate_simple(model, loader, device, dataset_name="Test"):
    """Simplified comprehensive evaluation with clinical common metrics"""
    model.eval()
    y_true, y_pred, y_prob = [], [], []
    with torch.no_grad():
        for batch in loader:
...
def virtual_screening(model, device):
    """Molecular virtual screening pipeline for active compound discovery"""
    # Load screening dataset with relative paths
    ...
    results = []
    selected_features = []
    target_dna_idx = 6  # Fixed DNA embedding index for screening

    # Iterate all candidate drugs for screening
    for drug_idx in range(len(screen_drug_feat)):
        ...
        dna_tensor = torch.FloatTensor(screen_dna_feat[target_dna_idx]).unsqueeze(0).to(device)
        ...
        # Reserve compounds with positive prediction
        if pred == 1:
            smiles = screen_smiles_df.iloc[drug_idx]['smiles']
            original_prob = screen_smiles_df.iloc[drug_idx]['probability']
            results.append({
                'drug_index': drug_idx,
                'dna_index': target_dna_idx,
                'smiles': smiles,
            ...
            print(f"Drug {drug_idx} passed screening threshold")
    ...
    if results:
        ...
        print(f"\nScreening completed! Valid active molecules count: {len(results)}")
        print(f"Screened SMILES results saved to: {SCREEN_RESULT_CSV_PATH}")
        print(f"Screened molecular features saved to: {SCREEN_RESULT_FEAT_PATH}")
    else:
        print("\nNo active molecules passed the screening threshold")
...
def main():
    ...
    labels = label_df['spr_label'].values
    ...
    # Stratified dataset split (Train/Val/Test)
    X_train_drug, X_test_drug, X_train_dna, X_test_dna, y_train, y_test = train_test_split(
        drug_feature_data, dna_feature_data, labels,
        test_size=TEST_SPLIT_RATIO, stratify=labels, random_state=SEED
    )
    X_train_drug, X_val_drug, X_train_dna, X_val_dna, y_train, y_val = train_test_split(
        X_train_drug, X_train_dna, y_train,
        test_size=VAL_SPLIT_RATIO, stratify=y_train, random_state=SEED
    )
    ...
    # Simplified comprehensive metric evaluation
    print("\n" + "="*40)
    ...
    # Execute molecular virtual screening
