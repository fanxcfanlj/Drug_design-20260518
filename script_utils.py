...
def read_and_convert_csv(file_path):
    ...
    for smile in smiles_data:
        try:
            selfies.encoder(smile)
            valid_smiles_data.append(smile)
        except selfies.exceptions.EncoderError:
            print(f"Invalid SMILES: {smile}, skipped")
    selfies_data = [selfies.encoder(smile) for smile in valid_smiles_data]
    return selfies_data
