...
from mol_gen_model.metrics.SA_Score import sascorer
from mol_gen_model.metrics.NP_Score import npscorer
from mol_gen_model.utils import mapper, get_mol
...
_pains = pd.read_csv(os.path.join(_base_dir, 'wehi_pains.csv'),
                     names=['smarts', 'names'])
combined = pd.concat([_mcf, _pains], ignore_index=True).sort_values(by='smarts')
_filters = [Chem.MolFromSmarts(x) for x in
            combined['smarts'].values]


def canonic_smiles(smiles_or_mol):
    '''
    三重防护：确保只返回合法的规范 SMILES
    '''
    try:
        # 第一重：获取有效分子（过滤解析失败、sanitize 失败的）
        mol = get_mol(smiles_or_mol)
        if mol is None:
            return None
        
        # 第二重：尝试 canonical 化，捕获底层断言错误
        canon_smi = Chem.MolToSmiles(mol, canonical=True, isomericSmiles=False)
        
        # 第三重：二次验证规范后的 SMILES 是否有效（避免极端情况）
        if Chem.MolFromSmiles(canon_smi, sanitize=True) is None:
            return None
        
        return canon_smi
    except Exception as e:
        # 捕获所有可能的异常（包括 RDKit 底层断言错误）
        # 可选：打印警告（注释掉可减少日志干扰）
        # print(f"Warning: Invalid SMILES '{smiles_or_mol}' (error: {str(e)[:50]})")
        return None
