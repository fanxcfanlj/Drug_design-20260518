...
def get_mol(smiles_or_mol):
    ...
    if isinstance(smiles_or_mol, str):
        if len(smiles_or_mol) == 0:
            return None
        # 显式开启 sanitize，过滤明显无效的分子
        mol = Chem.MolFromSmiles(smiles_or_mol, sanitize=True)
        if mol is None:
            return None
        try:
            # 严格 sanitize，检查化合价、化学键等
            Chem.SanitizeMol(mol, sanitizeOps=Chem.SanitizeFlags.SANITIZE_ALL)
        except (ValueError, RuntimeError):
            return None
        return mol
    elif isinstance(smiles_or_mol, Mol):
        try:
            Chem.SanitizeMol(smiles_or_mol, sanitizeOps=Chem.SanitizeFlags.SANITIZE_ALL)
        except (ValueError, RuntimeError):
            return None
